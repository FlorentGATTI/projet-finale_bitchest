...
function Wallet() {
  ...
  return (
    <div className="wallet-container bg-dark">
      <h2>Gérer le Wallet</h2>

      {/* Solde en euro */}
      <div className="balance-alert bg-items">
        Solde: {euroBalance}€
      </div>
      ...
      <div className="wallet-content bg-items">
        ...
        <ul className="crypto-list">
          {cryptos.map((crypto, index) => (
            <li key={index} className="crypto-item">{crypto.name}: {crypto.amount}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="sell-form bg-items">
        ...
        <div className="form-group">
          ...
          <select className="crypto-select" value={cryptoToSell} onChange={e => setCryptoToSell(e.target.value)}>
            {cryptos.map((crypto, index) => (
              <option key={index} value={crypto.name}>{crypto.name}</option>
            ))}
          </select>
        </div>
        ...
        <button className="sell-button" onClick={handleSale}>Vendre</button>
      </div>
    </div>
  );
}
